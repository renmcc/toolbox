...
import (
	...
	ecies "github.com/renmcc/toolbox/crypto/ecies/secp256k1"
...
func TestNewPrivateKeyFromHex(t *testing.T) {
	_, err := ecies.NewPrivateKeyFromHex(testingReceiverPrivkeyHex)
...
func TestPrivateKey_UnsafeECDH(t *testing.T) {
	privkey1, err := ecies.NewPrivateKeyFromHex(privkeyBase)
	if !assert.NoError(t, err) {
		return
	}
	for _, key := range privkeys {
		privkey2, err := ecies.NewPrivateKeyFromHex(key)
		if !assert.NoError(t, err) {
			return
		}
